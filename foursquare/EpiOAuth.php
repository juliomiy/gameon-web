...
//In case this is ever added like Twitter's sign in with
//public function getAuthenticateUrl($token = null, $params = null)
//{ 
//  $token = $token ? $token : $this->getRequestToken($params);
//  $values['url'] = $this->getUrl($this->authenticateUrl);
//  $values['oauth_token'] = $token->oauth_token;
//  $values['oauth_token_secret'] = $token->oauth_token_secret;
//  return $values;
//}
  ...
  {
    $token = $token ? $token : $this->getRequestToken($params);
    $values['url'] = $this->getUrl($this->authorizeUrl);
    $values['oauth_token'] = $token->oauth_token;
    $values['oauth_token_secret'] = $token->oauth_token_secret;
    return $values;
  }
